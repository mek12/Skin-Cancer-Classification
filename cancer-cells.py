# -*- coding: utf-8 -*-
"""
Created on Sat Jan  4 18:33:37 2020
...
1model.add(Conv2D(32,kernel_size=(3,3), activation="relu",padding="Same", input_shape=input_shape))
2model.add(Conv2D(32,kernel_size=(3,3), activation="relu",padding="Same"))
model.add(MaxPool2D(pool_size=(2,2)))
model.add(Dropout(0.25))


1model.add(Conv2D(64,kernel_size=(3,3), activation="relu",padding="Same"))
...
model.add(Dropout(0.5))
model.add(Dense(num_classes,ativation="softmax"))
model.summary()
...
window.wm_title("Skin Scanner Classification")
...
frame2= tk.LabelFrame(frame_right, text="Model and Save", width=540, height=140)
...
def openImage():
    ...
    if count != 1:
        ...
        image_name = filedialog.askopenfilename(initialdir="D:\codes", title="Select an image file")
...
columns = ["lesion_id", "img_id","dx","dx_type","age","sex","localization"]
...
def classification():
    ...
    if img_name != "" and models.get() !="":
        ...
        for i in range(len(h)):
            ...
            if i != h_index:
                tk.Label(frame4, text =str(h[i])).place(rely=x, rely=y)
            else:
                tk.Label(frame4, bg="green", text =str(h[i])).place(rely=x, rely=y)
...
classify_button=tk.Button(frame3, bg="red",bd104,font("Times",13), activebackround="orange", text="Classify", command =classification)
classify_button.place(relx=0.1,rely=0.5)

#frame4 sınıflandırma sonuçları
...
#frame2 
#combo box
model_selection_label=tk.Label(frame2, text="Choose classification mode: ")
model_selection_label.grid(row=0, column=0, padx=5)

models= tk.StringVar()
model_selection = ttk.Comcobox(frame2, textvariable=models, values=("Model1","Model2"), state="readonly")
